...
# --- 1. 載入模型與資料 ---
@st.cache_resource  # 快取模型資源，避免重複載入
def load_model():
...
def get_district_map():
    if os.path.exists('kaohsiung_main.csv'):  # 檢查資料檔案是否存在
        try:
            df = pd.read_csv('kaohsiung_main.csv', header=1)  # 嘗試讀取資料，跳過第一列
        except:
            df = pd.read_csv('kaohsiung_main.csv')  # 如果失敗，重新讀取資料
...
with st.container(border=True):  # 建立輸入容器
    ...
    with col1:  # 左欄
        selected_district_name = st.selectbox("📍 選擇行政區", options=sorted(name_to_code.keys()))  # 下拉選單選擇行政區
        district_code = name_to_code[selected_district_name]  # 取得選擇的行政區編碼
        house_age = st.slider("📅 目前屋齡 (年)", 0, 60, 10)  # 滑桿選擇屋齡
        
    with col2:  # 右欄
        total_ping = st.number_input("📐 權狀坪數", 5.0, 200.0, 35.0, 0.5)  # 輸入權狀坪數
        total_floors = st.number_input("🪜 總樓層數", 1.0, 50.0, 15.0, 1.0)  # 輸入總樓層數
        building_type_map = {"大樓/華廈": 3, "透天/別墅": 2, "公寓": 1}  # 建物型態對應表
        selected_type = st.radio("🏢 建物型態", list(building_type_map.keys()), horizontal=True)  # 單選按鈕選擇建物型態
        building_type = building_type_map[selected_type]  # 取得選擇的建物型態編碼
